...
for x in range(len(assoc)):
    for y in range(len(L1_size)):
        if assoc[x] == "full":
            command = "java sim_cache 32 {} {} 0 0 0 0 gcc_trace.txt".format(L1_size[y], int(L1_size[y]/32))
        else:
            command = "java sim_cache 32 {} {} 0 0 0 0 gcc_trace.txt".format(L1_size[y], assoc[x])
        output = subprocess.check_output(command, shell=True)
        output = output.decode("utf-8")
        val = re.findall("L1 miss rate:\\s+(\\d.\\d+)+", output)[0]
        miss_rates[x][y] = val
        aat_l1[x][y] = float(val)*100 + hit_times_32[y][x]
        print("Size: " + str(L1_size[y]) + " Assoc: " + str(assoc[x]) + " L1 Miss Rate: "+ str(val) + " AAT: " + str(aat_l1[x][y]))
    print("\n")
...
for x in range(len(rep_pol)):
    for y in range(len(L1_size)):
        command = "java sim_cache 32 {} 4 0 0 {} 0 gcc_trace.txt".format(L1_size[y], rep_pol[x])
        output = subprocess.check_output(command, shell=True)
        output = output.decode("utf-8")
        val = re.findall("L1 miss rate:\\s+(\\d.\\d+)+", output)[0]
        miss_rates[x][y] = val
        aat_l1[x][y] = float(val)*100 + hit_times_32_assoc_4[y][0]
        print("Size: " + str(L1_size[y]) + " Assoc: " + str(assoc) + " L1 Miss Rate: "+ str(val) + " AAT: " + str(aat_l1[x][y]))
    print("\n")
rp=["LRU", "FIFO", "OPT"]
print("\n\nTable 3:")
...
for x in range(len(inc_pol)):
    for y in range(len(L2_size)):
        command = "java sim_cache 32 1024 4 {} 8 0 {} gcc_trace.txt".format(L2_size[y], inc_pol[x])
        output = subprocess.check_output(command, shell=True)
        output = output.decode("utf-8")
        ...
        print("Size: " + str(L2_size[y]) + " L1 Miss Rate: " + l1_mr + " L2 Miss Rate: "+ l2_mr + " AAT: " + str(aat_l2[x][y]))
    print("\n")
...
for x in range(len(inc_pol)):
    ...
    plt.yticks(aat_l2[:][x])
    plt.plot(log_2_l2_size, aat_l2[:][x], label="RP "+ip[x])
plt.legend()
plt.show()